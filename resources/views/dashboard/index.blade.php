...
<!-- Search & Filters -->
<div class="mt-6 mb-4">
    <form action="{{ route('dashboard') }}" method="GET" class="flex flex-wrap gap-3">
        <!-- Search -->
        <div class="flex-1 min-w-[200px] relative">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search name, phone, email..." 
                class="w-full bg-[#111] border border-[#222] rounded-lg pl-10 pr-4 py-2.5 text-sm text-white focus:border-[#fe9e00] focus:outline-none">
        ...
        <!-- Client Filter -->
        <select name="client_id" onchange="this.form.submit()" class="bg-[#111] border border-[#222] rounded-lg px-4 py-2.5 text-sm text-white focus:border-[#fe9e00] focus:outline-none">
            <option value="">All Clients</option>
            @foreach($clients as $client)
                <option value="{{ $client->id }}" {{ request('client_id') == $client->id ? 'selected' : '' }}>
                    {{ $client->name }} ({{ $client->chats_count }})
                </option>
            @endforeach
        </select>
        
        <!-- Country Filter -->
        <select name="country" onchange="this.form.submit()" class="bg-[#111] border border-[#222] rounded-lg px-4 py-2.5 text-sm text-white focus:border-[#fe9e00] focus:outline-none">
            <option value="">All Countries</option>
            @foreach($countries as $country)
                <option value="{{ $country->country_code }}" {{ request('country') == $country->country_code ? 'selected' : '' }}>
                    {{ $country->country ?? $country->country_code }} ({{ $country->total }})
                </option>
            @endforeach
        </select>
        
        @if(request()->anyFilled(['search', 'client_id', 'country']))
        <a href="{{ route('dashboard') }}" class="px-4 py-2.5 bg-[#222] text-white rounded-lg text-sm hover:bg-[#333] transition-colors flex items-center gap-2">
